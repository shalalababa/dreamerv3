...
- `r3_ladder_read.py` — candidate-aware ladder L0–L4 on the committed R3 labels (registered-descriptive; frozen 31 Jul pre-outcome, `PREREG_r3_ladder_20260730`; substrate digest-gated; metric AMENDED pre-read to within-run Spearman by `PREREG_r3_ladder_amend1_20260731` after the independent review confirmed the pooled metric confounds run/domain structure with per-state legibility; awaiting amendment commit → ONE local read).
