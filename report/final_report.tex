...
\iclrfinalcopy % show author and de-anonymize for a course submission
...
\texttt{shalalababa1@gmail.com}}

\newcommand{\fig}[1]{../results/presentation_figures/#1}
...
\emph{what} property of the learned representation governs that transfer,
...
far more strongly with the \emph{coverage} of the pretraining data and with
...
\emph{continuously} by the learned exploratory policy, accumulating well after
its disagreement reward has collapsed, which rules out a sustained-uncertainty
mechanism. A protocol-calibration experiment shows the early-adaptation ranking
is preserved under standard full-adaptation, while a frozen-readout-specific
...
representation can be learned \emph{before} any task is specified, then reused
...
\emph{which} property of the learned representation is responsible. Exploration
...
controllable domain. We pretrain a DreamerV3 world model without task reward
under four strategies, adapt each to three Walker tasks, and pair the
adaptation curves with a battery of representation probes and a direct measure
of pretraining-data coverage. Our contributions are:
...
\emph{exploration objective} from the model and the downstream task.
...
time-resolved coverage analysis that rules out a sustained-uncertainty
explanation for Plan2Explore's benefit.
...
We are explicit throughout about statistical power: two seeds per condition and
six points per correlation. The results are best read as well-instrumented
observations that motivate specific follow-ups, not as significance claims.
...
task-specific. We use DreamerV3 because its objective cleanly separates the
task-agnostic world model from the task head, which is exactly the cut a
frozen-readout transfer study needs.
...
$k$-nearest-neighbor estimate of latent entropy \citep{singh2010nnentropy}.
Related signals include prediction-error curiosity \citep{pathak2017icm} and
random network distillation \citep{burda2019rnd}. Plan2Explore and APT are the
natural contrast for our central question: does \emph{uncertainty-driven}
exploration induce a different, better transferable representation than
\emph{coverage-driven} exploration?
...
Linear probes \citep{alain2017probes} are a standard tool for asking what a
...
\emph{current} state is near-trivial; we therefore emphasize future-state
...
\textbf{(RQ2)} Which property of the pretrained representation predicts
adaptation speed --- forward-predictive accuracy, decodability of state and
derived quantities, reward decodability, or data coverage?
...
the latent. C1 is the lower bound; C2 isolates ``a world model trained on
passive data'' from the effect of active exploration.
...
with logged MuJoCo physics, and encode it through \emph{each} condition's frozen
...
we measure \emph{coverage} of each condition's pretraining replay as a $k$-NN
...
protocol, we additionally run C3 (Plan2Explore) \emph{full-adaptation} on
...
\emph{sample efficiency} rather than asymptotic performance at this budget. Two
...
\subsection{RQ2: Coverage and derived-quantity probes predict transfer; current
state does not}
...
We stress three caveats. First, with $n=6$ these correlations are descriptive,
...
\subsection{RQ3: Plan2Explore $\approx$ APT, and the benefit is not a sustained
uncertainty signal}
...
transfer here. Notably, the uncertainty-driven method (Plan2Explore) produced
the \emph{highest} data coverage, more than the coverage-driven method (APT),
...
The coverage is thus generated continuously by the \emph{learned exploratory
policy}, not by a sustained intrinsic reward and not as an early burst that is
later merely retained. This rules out a sustained-uncertainty-signal account of
the benefit. It is consistent with coverage-mediation, but does not prove it,
...
\subsection{Protocol calibration: frozen-readout validates the headline, not the
final-performance deficit}
\label{sec:calib}

Table~\ref{tab:calib} compares C1 and C3 on \texttt{walk} under both protocols.
On the primary metric the two protocols \emph{agree}: C3 beats C1 early under
frozen-readout ($+200$ AUC) and by even more under full-adaptation ($+382$). The
headline ``exploratory pretraining speeds adaptation'' is therefore not a
frozen-readout artifact. On final return the ordering \emph{flips}: under
freezing C3 trails C1 by $219$, but under full-adaptation C3 \emph{leads} by
$191$ and reaches near-ceiling ($945$). The frozen-readout final deficit (and,
by extension, C2's collapse) reflects the cost of holding the world model fixed,
not a deficiency of the pretrained dynamics. Plan2Explore's world model is a
...
\begin{table}[t]
\centering
\caption{Protocol calibration on \texttt{walk} (2 seeds). The early-AUC ranking
is protocol-invariant; the final-return ordering is not.}
\label{tab:calib}
\begin{tabular}{lcc}
\toprule
...
representations, what representational property governs the transfer, and
...
exploration transfer comparably (RQ3), and a time-resolved analysis rules out a
sustained-uncertainty mechanism for Plan2Explore: its coverage is built
continuously by the learned policy, even after its disagreement reward
collapses. A minimal protocol calibration shows the speed ranking holds under
standard full-adaptation, while a frozen-readout-specific final-performance
deficit disappears.

\paragraph{Implications.}
Three points generalize beyond Walker. First, in proprioceptive control the
representation property that tracks transfer is not how linearly decodable the
current state is --- that saturates --- but the data distribution the
exploration policy induces (which here we cannot cleanly separate from simply
reaching the task-relevant regime). Either way, this argues for evaluating
...
nuisance: frozen-readout is a conservative lower bound that can invert
...
locomotion'' --- for example by measuring coverage restricted to upright,
moving states --- would clarify whether exploration helps by breadth or by
reaching the right regime.

\subsubsection*{Reproducibility and disclosure}
All world-model pretraining, adaptation, probing, coverage, and synthesis code,
together with the per-seed metric logs and the figures reproduced here, are in
the project repository. Reported numbers are computed by the probing pipeline
(\texttt{probing/}) from the logged run metrics; tables cite condition means
with per-seed spread. The author used an AI coding assistant for implementation
scaffolding, experiment orchestration, and drafting support; all experimental
design decisions, result interpretation, and final claims are the author's, and
every citation was checked against its source.
