...
\emph{which} property of the learned representation tracks that transfer,
...
\emph{single} Walker body and proprioceptive observation space, not
...
uncertainty-versus-coverage contrast within a \emph{single} world-model agent
...
the latent. C1 is the no-pretraining baseline (not a strict lower bound --- at
the adaptation budget it can beat a frozen pretrained model on final return);
...
later merely retained. This argues against a sustained, high-magnitude
...
On the primary metric the two protocols \emph{agree} for this C1-vs-C3 contrast:
...
artifact \emph{for the one contrast we calibrated}. We do not claim
protocol-invariance beyond C1-vs-C3 on \texttt{walk}. On final return the
ordering \emph{flips}: under freezing C3 trails C1 by $219$, but under
full-adaptation C3 \emph{leads} by $191$ and reaches near-ceiling ($945$). The
