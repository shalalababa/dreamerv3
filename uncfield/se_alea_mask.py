"""B4 aleatoric-repair interventional mask instrument
(PREREG_trackB_alea_20260822.md; built + selfchecked BEFORE the B4
wave's compute).

Measures the DEPLOYED accounting of a gauss-head run on masked
replay anchors — BOTH statistics from one pass over identical
variants:
  raw_rew  = var over members of mu           (the Stage-1 pricing)
  norm_rew = var(mu) / (ensemble-mean predicted aleatoric var)
             (the B4 repair — the arm's actual deployed reward)
Mask suite verbatim from the frozen reviewed se_mask/se_apt_mask
line: distractor -> batch permutation (the fire channel), dup0 ->
bitwise no-op anchor, dup1/dup2 -> substitution + fresh-resample
(variance-matched), velocity -> batch-permutation calibration
control. Population-intervention estimand; anchor-level p/BCa
diagnostic-not-calibrated; S pinned.
...
def run(args):
    ...
    np.savez(os.path.join(out_dir, "se_alea_mask.npz"),
             ...
             base_norm_stoch=base["norm_stoch"], **dnpz)
...
def selfcheck():
    ...
    print("se_alea_mask selfcheck PASS (gauss ckpt; dup0 zero on "
          "both statistics; finite; resample+velocity variants; "
          "gauss diagnostics recorded; deterministic)")
