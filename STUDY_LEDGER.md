...
- **Pixel swamping diagnostic** (registered 07-24 + **Amend 1 07-25**, 1 GPU job to RESUBMIT): first E4 job crashed pre-output (`KeyError: 'image'` — E4 measure's proprio-era key filter starved the pixel encoder; probe set itself carries image and stays FROZEN); fix in `probing/stratified_error.py` keyed off the model's own enc/dec spaces (proprio numbers bit-identical), read frozen pre-outcome (`analysis/pixel_swamping_read.py`, selfcheck PASS; trivial-floor convention + apt absent-by-design pinned in `PREREG_pixel_swamping_amend1_20260725`). Resubmit same sbatch command, then ONE read.
...
- **Paper 4 collusion pilot — stage 1 GO** (07-25, `artifacts/collusion_stage1_20260725/`): mean Δ_profit 0.831 (published ≈0.7–0.9 ✓), fingerprint 68/100 ✓; failure-to-learn minority present (high-Δ no-fingerprint) ⇒ confirmatory measurement must be fingerprint-conditional. **Stage-2 finding: coverage saturated at 1.000 in ALL sessions ⇒ RQ1 matched-(Cov,Occ) pairs NOT constructible from 2M logs — coverage knob (session length / ε schedule / grid m) must be added before the confirmatory prereg.** Next: stage-2 regime-definition analysis on this artifact + Design-B viability probe build + small coverage-knob pilot.
