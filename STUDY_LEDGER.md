...
| 08-02 | **NFI mechanism analysis** (instrumented replay of 8 cycles/member on the frozen pilot2 ensemble) | — (exploratory follow-up) | `artifacts/nfi_mechanism_20260802/` | **Three exploit anatomies:** (1) belief-update over-contraction vs the model's OWN heads (EIG 1.4–11.6× the head-licensed Bayes gain; heads individually sane — R_implied≤0 nowhere — the UPDATE OPERATOR is the incoherent part); (2) pure fictitious information on TV (head-licensed gain exactly 0, realized EIG +0.84); (3) off-support leakage (contraction on unmeasured components ≥ on-support) vs duplicate double-count (chord loop: on-support only) ⇒ **candidate referee-free diagnostic: the self-consistency ratio (EIG ÷ own-heads gain)** — being tested across the sensitivity sweeps; m2 contrast = same defects, sub-threshold amplitude |
