...
- **Spectral domain-contrast pass** (registered 07-24, to run on login node/CPU): `PREREG_spectral_domains_20260724` + **Amend 1** (`PREREG_spectral_domains_amend1_20260724`: θ-inestimable buffers → spectrum-only records + dropped-side disclosure + cross-buffer θ secondary; instrument v1_1, version-gated ⇒ re-measure all buffers) → `probing/spectral_measure.py` (selfcheck PASS); P-SM1 = cup reward-direction variance rank < finger, every evaluable matched side.
