...
- **Paper 4 confirmatory sweep REGISTERED 07-30** (to submit after freeze-commit): `PREREG_collusion_confirm_20260730` → `analysis/collusion_confirm_read.py` (selfcheck PASS; strict csv hygiene — nan/malformed cells halt the read). Calibration `artifacts/collusion_calib_20260730/`: reference frozen = TERMINAL distance label (exo degenerate); harness `collusion_pilot_v3_20260730` (Design-A biased exploration, budget-matched) + `collusion_designb_v2_20260730` (graded volume-controlled down-weighting, dose 1 ≡ deletion). Design B seeds 1000–1099 (100 recorded sessions × doses .25/.5/1 × size-matched random controls, exact-replay gate); Design A 5×100 arms seeds 2000–3399. Primaries: P-CLB1/2 fp-cond composition increments (CI<0); **P-CLA1/2 re-specified PRE-FREEZE from the calibration smoke as a DISSOCIATION** (forbid λ=1: fingerprint collapses 0/10 while Δ rises +0.987 vs +0.844 — induced Abada–Lambin regime; fp CI<0 + Δ CI>0). Two adversarial review passes (10 + 11 findings, incl. 2 blocking reader-corruption defects) all fixed pre-freeze; ~5 core-h CPU total; ONE read in the Paper-4 chat.
