...
- **Volume-anomaly replication (P-E4a)** — UNBLOCKED by the 07-26 Scaling B read; needs its own dated registration BEFORE submission (~12 jobs, v400s1-type vs v200s1-type at n=6→12).
- **Paper-3 confirmatory prereg** — freezes next per the launch plan: 25m buy rule targets the A→B transition (task-lo rise), parameterized by `artifacts/scaling_optionb_20260726/`.
