"""DERIVED READER — POOLED secondary, 64 cells, NO decision weight
(PREREG_p2_wave2_freshcohort_20260821, review B3). A literal copy of the
executed frozen analysis/w1_read.py with ONLY the changes listed below
(the frozen original is NOT edited, per house rule); the prereg records
this file's sha256 and the exact diff.
Changes vs w1_read.py:
  1. FILE_RES[tm2] seed group: 5[1-8] -> (?:5[1-8]|59|6[0-6])
  2. DUP_RES[tm2] seed group:  same union
  3. EXPECT_CELLS: 32 -> 64
  4. output name: w1_{family}.json -> w1pooled_{family}.json
"""
...
FILE_RES = dict(
    ...
    tm2=re.compile(r'^w1tm2_(cup|finger)_(e1|e4)_seed(?:(5[1-8]|59|6[0-6]))_late\.npz$'))
DUP_RES = dict(
    dv3=re.compile(r'^w1dup_(cup|finger)_(e1|e4)_seed(3[1-8])_late\.npz$'),
    tm2=re.compile(r'^w1tm2dup_(cup|finger)_(e1|e4)_seed(?:(5[1-8]|59|6[0-6]))_late\.npz$'))
...
def run_read(labels_dir, family, output):
  ...
  path = os.path.join(output, f'w1pooled_{family}.json')
