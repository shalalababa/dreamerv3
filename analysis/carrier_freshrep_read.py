...
ALPHA_FRESH = 0.05
ALPHA_POOLED = 0.016667                # 7 * .05 / 21 (BH re-entry target)
UPDATES = 500000
...
def run(args):
  ...
  fresh_fire = fresh['perm_p'] < ALPHA_FRESH
  reentry = pooled['perm_p'] <= ALPHA_POOLED
  if fresh_fire and reentry:
...
def selfcheck():
  ...
  for effect, expect in ((120.0, 'CARRIER-REPLICATES'),
    ...
    auc, wit = _fixture(base, effect, seed=1)
    args = argparse.Namespace(auc=auc, witness=wit, look1=None,
  ...
  for mutate in ('short_counters', 'missing_fit', 'sha_split', 'no_meta',
                 'qc_fail', 'nep_drift', 'bad_look1'):
    base = tempfile.mkdtemp(prefix='cfr_sc_')
    auc, wit = _fixture(base, 100.0, seed=2)
    ...
    try:
      run(argparse.Namespace(auc=auc, witness=wit, look1=look1,
                             output=os.path.join(base, 'out')))
    except Refusal as e:
      ok_refusals += 1
  ...
  assert ok_refusals == 7
  print('carrier_freshrep_read selfcheck PASS (2 branches + 7 refusals; '
...
def main():
  ...
  if args.selfcheck:
    ...
    assert args.auc and args.witness and args.output
    run(args)
